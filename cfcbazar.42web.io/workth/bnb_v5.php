...
// 4) Compute unique amount: 0.004{ID}{ID}
if ($userId !== null) {
...
// 6) Handle walletaddress form submission
$address_error = '';
...
// 7) Fetch any saved worker address (default to empty string)
$existing_address = '';
$stmt = $conn->prepare("
    SELECT address
      FROM workers
...
$res = $stmt->get_result();
if ($row = $res->fetch_assoc()) {
    // cast to string so trim() never gets null
    $existing_address = trim((string)$row['address']);
}
$stmt->close();

// 8) Handle AJAX “check” for deposit
if ($existing_address
    && isset($_GET['check'])
...
<?php if (!$existing_address): ?>
  ...
  <form method="POST">
    <input
      type="text"
      name="wallet_address"
      placeholder="0xYourBnbWalletAddress"
      required
    />
  ...
  <?php endif; ?>
  <div class="note">We need your BNB address so you can deposit the unique amount.</div>
