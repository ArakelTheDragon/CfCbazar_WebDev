    ...
    if ($data && $data['status'] === '1') {
        foreach ($data['result'] as $tx) {
            ...
            if ($to === strtolower($bnb_wallet) && abs($value - $expected) <= 0.00001) {
                // Check if the tx_hash is already in deposit_amounts for this user
                $checkTx = $conn->prepare("SELECT 1 FROM deposit_amounts WHERE user_email = ? AND tx_hash = ? LIMIT 1");
                $checkTx->bind_param("ss", $email, $hash);
                $checkTx->execute();
                $checkTx->store_result();

                if ($checkTx->num_rows > 0) {
                    echo " You have already been credited for this TX.";
                    exit;
                }
                $checkTx->close();

                // Credit user
                $up = $conn->prepare("UPDATE workers SET tokens_earned = tokens_earned + 100 WHERE email = ?");
                $up->bind_param("s", $email);
                $up->execute();
                $up->close();

                // Insert into deposit_amounts table
                $insert = $conn->prepare("INSERT INTO deposit_amounts (user_email, amount, token, address, created_at, used, tx_hash) VALUES (?, ?, 'bnb', ?, NOW(), 1, ?)");
                $insert->bind_param("sdss", $email, $value, $from, $hash);
                $insert->execute();
                $insert->close();
...
<head>
  ...
  <style>
    ...
    h2 { color: #1e40af; }
    .card { background: #f4f7ff; padding: 12px 16px; border-radius: 10px; position: relative; word-break: break-word; }
...
<main>
  ...
  <div id="countdown" style="display:none"> Time left: <span id="timer">300</span>s</div>
