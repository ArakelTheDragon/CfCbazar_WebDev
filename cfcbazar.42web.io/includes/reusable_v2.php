...
function include_menu() {
    ...
    echo '<button class="menu-toggle" aria-expanded="false" aria-label="Open menu">☰</button>';
    echo '<ul class="nav-menu">';
    echo '<li><a href="/index.php">🏠 Home</a></li>';
    echo '<li><a href="/d.php" target="_blank">💰 WorkToken</a></li>';
    echo '<li><a href="/pow/" target="_blank">⛏️ Mine WorkTokens/WorkTHR</a></li>';
    echo '<li><a href="https://fb.com/workthrp" target="_blank">🔗 Smart Deals</a></li>';
    echo '<li><a href="/features.php" target="_blank">🔧 DIY Tools</a></li>';
    echo '<li><a href="/games.php" target="_blank">🎮 Games</a></li>';
    echo '<li><a href="https://youtube.com/playlist?list=PLY4e42xsZig5Yu7GZ6VN1OSn-0cy90yJu" target="_blank">🎵 Music</a></li>';
    echo '<li><a href="/login.php">🏠 Sign In / Register</a></li>';
    echo '<li><a href="https://ebay.us/m/DM1tRs" target="_blank">🚚 Visit Store</a></li>';
    echo '<li><a href="https://github.com/ArakelTheDragon/CfCbazar-Tokens" target="_blank">📖 About WorkToken</a></li>';
    echo '<li><a href="/help/" target="_blank">❓ Help Center</a></li>';
    echo '<li><a href="/about.php">ℹ️ About Us</a></li>';
    echo '<li><a href="/t.php">📜 Terms & Privacy</a></li>';
    echo '<li><a href="/c.php">🍪 Cookies</a></li>';
...
function showAdvertPopup() {
    ...
    <div id="advertPopup" style="display:none; position:fixed; bottom:20px; right:20px; width:300px; background:#fff; border:1px solid #ccc; box-shadow:0 0 10px rgba(0,0,0,0.3); padding:15px; z-index:9999;">
        ...
        <strong>🔥 Introduction to CfCbazar:</strong><br>
...
function trackVisit(mysqli $conn): void {
    ...
    if (preg_match('/bot|crawl|spider|preview|monitor/i', $ua)) {
        return; // skip bots
    }

    $ref = $_SERVER['HTTP_REFERER'] ?? null;

    // Referrer-based override for ambiguous /index.php
    if ($path === '/index.php') {
    ...
    if (!isset($_COOKIE[$cookieName])) {
        ...
        if ($upd) {
            $upd->bind_param('ss', $ref, $path);
            $upd->execute();

            if ($upd->affected_rows === 0) {
                $slug = trim($path, '/');
                $slug = $slug === '' ? 'home' : $slug;
                $title = ucfirst(str_replace('.php', '', $slug));
                $status = 'published';
                ...
                ");
                if ($ins) {
                    $ins->bind_param('sssss', $title, $slug, $path, $status, $ref);
                    $ins->execute();
                    $ins->close();
                }
            }
...
function redirectToCfCbazar42web() {
    header("Location: https://cfcbazar.42web.io/");
    exit;
}
