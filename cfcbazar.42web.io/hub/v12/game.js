  ...
  fetch(`https://cfcbazar.atwebpages.com/get_tokens.php?email=${encodeURIComponent(email)}`)
    ...
    .then(data => {
      ...
      if (data.error) {
        ...
        display = tokens.toFixed(2);

        // Redirect user to miner if tokens are zero or less
        if (tokens <= 0) {
          alert("⚠️ You have 0 WorkTokens. Please mine some tokens first.");
          window.location.href = "https://cc.free.bg/site/miner/";
          return;
        }
      }
...
function rollDice(difficulty) {
  ...
  const reward = 0.1 * playerLevel;
  const penalty = 0.2 * playerLevel;

  if (dice >= difficulty) {
    result += ` ✅ Success! +${reward.toFixed(2)} WorkTokens`;
    updateTokens(reward);
    gainXP(10); // XP on success
    document.getElementById("answerSection").style.display = "block";
  } else {
    result += ` ❌ Failed! -${penalty.toFixed(2)} WorkTokens`;
