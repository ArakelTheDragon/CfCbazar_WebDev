...
<body>
  <div class="card">
    ...
    <div>
      <!-- readonly prevents mobile keypad -->
      <input type="text" id="guess" maxlength="1" placeholder="?" readonly />
  ...
  <script>
    ...
    function buildKeyboard() {
      ...
      "abcdefghijklmnopqrstuvwxyz".split("").forEach(letter => {
        ...
        btn.addEventListener("click", () => handleKeyClick(letter, btn));
        container.appendChild(btn);
      });
    }

    function handleKeyClick(letter, btn) {
      makeGuess(letter);
      btn.disabled = true;
    }

    function makeGuess(forcedLetter) {
      const letter = forcedLetter || input.value.toLowerCase().trim();
