...
function getUserStatus(?string $email = null): int
{
    ...
    // --- SAFETY PATCH ---
    // If a mysqli object was passed instead of an email → auto-fix it
    if ($email instanceof mysqli) {
        $email = $_SESSION['email'] ?? null;
    }

    // If email is missing → user is not logged in
    if ($email === null || $email === '') {
        return 0;
    }
...
function checkSystemFlags(): bool {
    ...
    if ((int)$settings['maintenance'] === 1) {
        // Prevent infinite redirect loop if already on the maintenance page
...
function show_disabled_message(string $reason = 'maintenance'): void {
    ...
    echo '<p>For more, please visit our <a href="/system/news.php" target="_blank">News Center</a>.</p>';
    echo '</div>';
    echo '</div>';
...
<?php
// helper.php for /diy/track/index.php
/**
 * ============================================================================
...
{
    global $conn;
   // Re-sync tracking database entries to index.json
updateTrackingJson();
 ...
 */
 ...
 // reusable functions
 /**
 * Render the PancakeSwap Trade WorkTokens card module.
 */
...
/**
 * Fetches the path for a given page slug.
 *
 * @param string $slug The page slug
 * @param string $fallback Default URL if not found
 * @return string The URL path
 */
function rvPageRedirect($slug, $fallback = '#') {
    global $conn;
...
/* Usage
<a href="<?= fGetPageUrl('diy-power') ?>">Go to DIY Power Page</a>
*/
 ...
 * --------------------------------------------------------------------------
 * Fetches all records from the `tracking` table and updates /diy/track/index.json
 *
 * @param string|null $filePath Custom target path (defaults to /diy/track/index.json)
...
function updateTrackingJson(?string $filePath = null): bool
{
    ...
    if (!($conn instanceof mysqli)) {
        error_log("updateTrackingJson(): Database connection is not available.");
        return false;
    }

    // Determine target path relative to document root if not specified
    if ($filePath === null) {
        $docRoot = $_SERVER['DOCUMENT_ROOT'] ?? __DIR__ . '/..';
        $filePath = rtrim($docRoot, '/\\') . '/diy/track/index.json';
    }
